...
public class EmailUtil {
    ...
    public static String myEmailPassword = "xadglqpkferwigaa";
    ...
    public static String sendEmail(String to) {
        ...
        MimeMessage message;
        try {
            message = createMimeMessage(session, myEmailAccount, to, verification);
    ...
    public static MimeMessage createMimeMessage(Session session, String sendMail, String receiveMail, String code) throws Exception {
        ...
        message.setFrom(new InternetAddress(sendMail, "GHD_Shopping", "UTF-8"));
        // 3. To: 收件人（可以增加多个收件人、抄送、密送）
        message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(receiveMail, "用户", "UTF-8"));
        // 4. Subject: 邮件主题（标题有广告嫌疑，避免被邮件服务器误认为是滥发广告以至返回失败，请修改标题）
        message.setSubject("欢迎您使用GHD_Shopping APP，以下是您的验证码！", "UTF-8");
